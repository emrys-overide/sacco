# Sowetamu SACCO officer roles and responsibilities

The server, not the navigation menu, enforces these permissions. A role only
has the authority listed below. The Chairman should create an individual account
for every officer; officers must not share passwords.
...
| Member | Personal self-service | View only their own profile, vehicle, transactions, receipts, payments, loans, rejection reasons, and settings; apply or reapply for their own loan; update own email/profile photo/password. | View another member's data, officer data, the SACCO ledger, bank events, reports, or change SACCO records. |

## Loan decision sequence

1. A member submits the application, choosing a repayment period in months.
2. The Secretary confirms membership and eligibility requirements.
3. The Treasurer checks the financial figures and repayment ability.
4. The Chairman makes the final approval/disbursement decision.

Every rejection must include a clear reason. The member sees that reason in
...
**Maximum principal** is the highest original loan amount that may be requested
under the SACCO policy. It does **not** include interest, penalties, or the
total amount the member will repay. For example, a maximum principal of KES
100,000 blocks an application for KES 100,001 even if its eventual repayment
total would be higher because of interest. Leave it blank only if the SACCO has
no application-level cap; other eligibility rules still apply.
